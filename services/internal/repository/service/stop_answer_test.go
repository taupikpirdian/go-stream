...
func Test_chatRepository_StopAnswer(t *testing.T) {
	type fields struct {
		client *http.Client
	}
	type args struct {
		ctx     context.Context
		reqData *entity.StopReq
	}
	...
	var (
		...
		reqData = &entity.StopReq{
			...
			RequestId: "533be47a-9091-4524-afc9-38a9c32447e2",
			TaskId:    "533be47a-9091-4524-afc9-38a9c32447e3",
		}
		response = `{
			"result": "success"
		}`
	...
	akeClientResponseErrorNotCriteria := &http.Client{Transport: testdata.RoundTripFunc(func(req *http.Request) *http.Response {
		return &http.Response{
			StatusCode: http.StatusOK,
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				client: akeClientResponseErrorNotCriteria,
			},
			args: args{
				ctx:     ctx,
				reqData: reqData,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Run(tt.name, func(t *testing.T) {
				...
				err := repo.StopAnswer(tt.args.ctx, tt.args.reqData)
				if tt.wantErr {
					assert.Error(t, err)
					return
				}
