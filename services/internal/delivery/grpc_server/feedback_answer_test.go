...
func Test_chatServiceHandler_FeedbackAnswer(t *testing.T) {
	var (
	...
	ucChatBotGenAi_Error.On("FeedbackAnswer", mock.Anything, mock.Anything).
		Times(1).
		Return(errors.New("error feedback answer"))

	ucChatBotGenAi := new(mocks.ChatUseCase)
	ucChatBotGenAi.On("FeedbackAnswer", mock.Anything, mock.Anything).
		Times(1).
		Return(nil)
	...
	type args struct {
		ctx context.Context
		req *chatPb.FeedbackAnswerRequest
	}
	tests := []struct {
		name    string
		fields  fields
		...
		{
			name: "error - usecase feedback answer",
			fields: fields{
			...
			args: args{
				ctx: ctxValid,
				req: &chatPb.FeedbackAnswerRequest{
					MessageId: "799c209a-4805-4c77-b43d-b4361c2b6af4",
					Feedback:  "like",
				},
			},
			want:    nil,
		...
		{
			name: "success - usecase feedback answer",
			fields: fields{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.FeedbackAnswer(tt.args.ctx, tt.args.req)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
